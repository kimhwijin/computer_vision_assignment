...
def create_kfold_train_validation_dataframe(df:pd.DataFrame)->tuple(pd.DataFrame, pd.DataFrame):
    df = __encode_flags_to_string(df)
    train_df, valid_df = __split_kfold_train_validation_and_sampling_dataframe(df)
    return train_df, valid_df

def __encode_flags_to_string(df:pd.DataFrame)->pd.DataFrame:
...
def __split_kfold_train_validation_and_sampling_dataframe(df:pd.DataFrame)->tuple(pd.DataFrame, pd.DataFrame):
    ...
    fold_train_df["large_bowel_RLE_encoded"].fillna("", inplcae=True)
    fold_train_df["small_bowel_RLE_encoded"].fillna("", inplcae=True)
    fold_train_df["stomach_RLE_encoded"].fillna("", inplcae=True)


    fold_valid_df["large_bowel_RLE_encoded"].fillna("", inplcae=True)
    fold_valid_df["small_bowel_RLE_encoded"].fillna("", inplcae=True)
    fold_valid_df["stomach_RLE_encoded"].fillna("", inplcae=True)
